...
async function saveTeams(teams) {
  if(await getTeamsByKey(teams.id) == undefined) {
    ...
    .then(function() {
      alert("Data berhasil difavoritkan.");
    });
  } else {
    alert("Data sudah masuk dalam daftar favorit.");
  }
}
...
async function saveSchedule(schedule) {
  if(await getScheduleByKey(schedule.id) == undefined) {
    ...
    .then(function() {
      alert("Berhasil ditambahkan ke jadwal nonton.");
    });
  } else {
    alert("Jadwal sudah ditambahkan ke jadwal nonton.");
  }
}
...
function deleteSchedule(id) {
  dbPromised.then(function(db) {
    ...
    alert("Jadwal nonton berhasil dihapus");
  });
}